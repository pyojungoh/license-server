            ...
            if not match:
                ...
                if is_expired and is_user_match:
                    error_msg = (
                        ...
                        f"⚠️ 중요: 모바일 앱에서 '토큰 전송'만 하지 말고, 반드시 로그아웃 후 다시 로그인해야 새 토큰이 생성됩니다.\n\n"
                        f"다음 순서로 진행해주세요:\n\n"
                        f"1. 모바일 앱 실행\n"
                        f"2. 모바일 앱에서 로그아웃\n"
                        f"3. PC 프로그램과 같은 아이디({self.current_user_id})로 다시 로그인\n"
                        f"4. 모바일 앱에서 블루투스 기기로 사용자 정보 전송\n"
                        f"5. PC 프로그램에서 다시 시도\n\n"
                        f"※ 토큰은 7일 동안 유효하며, 만료되면 로그인을 다시 해야 새 토큰이 생성됩니다."
                    )
